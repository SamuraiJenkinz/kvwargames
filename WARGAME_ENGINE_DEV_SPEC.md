# War Game Engine — Development Specification
**Document version:** 1.0  
**Date:** April 2026  
**Purpose:** Complete implementation specification. Read this document in full before writing any code.
...
The application must run against a **configurable corporate LLM endpoint** — not an external public API. The LLM endpoint, API key, model name, and any corporate-specific headers must be configurable via environment variables.
...
- Configurable LLM backend (corporate API endpoint, not hardcoded to one vendor)
...
Quick reference to verify the system prompt captures each persona correctly.
