...
export default function CardsTab() {
  ...
  const cards = useGameStore((s) => s.gameConfig?.cards ?? [])

  if (selectedId !== null) {
    const card: GameCard | undefined = cards.find((c) => c.id === selectedId)
    ...
    <ul className="space-y-1">
      {cards.map((card) => (
        <li key={card.id}>
          ...
          >
            <span
              className={`w-1 self-stretch rounded-sm flex-none ${catChipClass(card.cat)}`}
            />
