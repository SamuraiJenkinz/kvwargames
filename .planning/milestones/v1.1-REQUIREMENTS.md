# Requirements Archive: v1.1 Pre-live-run hardening
...
- [x] **DEBRIEF-01**: Facilitator input text captured in debrief export preserves the first character (no "ound 1..." truncation) (closed 2026-04-15 — not reproducible in pure function pipeline; browser/OS download artifact from v1.0 live run; regression test in `debriefExporter.test.ts` pins the invariant)
...
- [x] **PROMPT-03**: Verified empirically via replay of the v1.0 Scenario 2 live run (severity=4 triggers the transition) (closed 2026-04-15 — Tier B live-LLM replay PASS recorded at `.planning/phases/12-crisis-state-prompt-engineering/12-LIVE-VERIFICATION.md`)
...
**Shipped:** 18 of 18 v1.1 requirements — 100% coverage.
**Adjusted:** None. All requirements landed as originally scoped; DEBRIEF-01 closed via Branch B (regression guard + browser-artifact note) rather than a source edit, but the requirement text was not rewritten.
**Dropped:** None.

---
*Archived: 2026-04-15 as part of v1.1 milestone completion*
