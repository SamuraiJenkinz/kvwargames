...
**The store's `addMessage` + `setLoading` can compose these stubs.** Whether they become dedicated store actions or are handled inline in the component is discretion (inline composition is simpler for Phase 5 stubs). The planner should note that Phase 6 will need them as store actions — pre-creating them with stub implementations avoids churn.
...
### 3. Legitimacy Track Bar Visual Direction (LOW priority, discretion)
