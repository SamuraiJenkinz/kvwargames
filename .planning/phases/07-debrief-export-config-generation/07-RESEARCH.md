...
This matches the "Discretion" note in CONTEXT.md — config generation is independent of the main LLM turn loop.
...
3. **Backend `response_format: json_object` — model compatibility**
   ...
   - What's unclear: if the deployment uses a non-OpenAI-compatible endpoint (e.g. alternative vendors, or a local model), `response_format` may be silently ignored or cause a 400 error
